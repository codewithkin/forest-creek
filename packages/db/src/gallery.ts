 ...
 * becomes a one-photo gallery. Rooms and activities both carry that pair.
 */
export function syncGallery<T extends { image?: string; images?: string[] }>(data: T): T {
...
/** A row's photos, cover first, for rows written before galleries existed too. */
export function galleryImages(room: { image: string; images: string[] }): string[] {
